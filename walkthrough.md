...
1. **The Widget (`Markdown.java`)**: We designed a stateful component that receives `Markdown.State` (holding the raw string) and the parent terminal `Rect` constraints.
2. **The AST Visitor (`RenderVisitor`)**: We extended CommonMark's `AbstractVisitor` to traverse the resulting Node tree element by element. At each node (e.g. `Heading`, `List`, `Text`), it delegates TamboUI `Cell` printing while persisting and modifying the current active `Style`.
